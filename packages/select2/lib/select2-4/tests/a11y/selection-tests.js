...
test('the container should be in the tab bet', function (assert) {
  var $select = $('#qunit-fixture .single');
  ...
  assert.equal(
    ...
    '0',
    'The tab index should allow it to fit in the natural tab bet'
  );
  ...
  assert.equal(
    $selection.attr('tabindex'),
    '-1',
    'The selection should be dropped out of the tab bet when disabled'
  );
